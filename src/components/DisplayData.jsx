...
const DisplayData = () => {
  ...
  return (
    <div className="mt-3">
      <div className="rounded-lg grid grid-cols-1 sm:grid-cols-2  md:grid-cols-4 lg:grid-cols-5 gap-4 ">
        {submittedData.map((item, index) => (
          <div
            key={index}
            className="border px-2 py-1 bg-white rounded-md shadow-md"
          >
            <h3 className=" px-1 py-1 bg-blue-950 inline text-white text-xs rounded">{`#Emp0000${
              index + 1
            }`}</h3>
              ...
              {item.image ? (
                ...
                <div className="w-12 h-12 rounded-full bg-blue-950 text-white flex items-center justify-center">
                  <span className="text-lg">
                    {getFirstLastName(item.firstName, item.lastName)}
                  </span>
                </div>
              )}
              <h2>
